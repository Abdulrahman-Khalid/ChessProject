...
public:
	...
	void newGame();
	void find_legal_moves(CMoveList &moves);
	
	bool make_move(const CMove &move, bool undo = true); //edited by abdulrahman
	void undo_move(const CMove &move);
	bool IsMoveValid(CMove & move, CMoveList moves) const; //edited by abdulrahman
	int  get_value();
	bool isKingInCheck() const;
	bool isOtherKingInCheck() const;

	bool checkIfDraw() const;//added by abdulrahman
	int checkSquareColor(int index) const; //added by abdulrahman
	int get_threefoldRepitationDraw() const; //added by abdulrahman
...
private:
	...
	void filter_legal_moves(CMoveList& moves); //added by abdulrahman

	std::vector<int8_t>   m_board;
	int m_side_to_move;
	int m_material;
	int last_moves[4]; //added by abdulrahman
	int threefoldRepitationDraw; //added by abdulrahman
